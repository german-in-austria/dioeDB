...
class tbl_fragebogen(models.Model):
	...
	FG_SK_1				= models.IntegerField(choices=[(i, i) for i in range(1, 6)], blank=True, null=True					, verbose_name="FG: Ich habe genau so gesprochen, wie ich auch sonst mit dieser Person spreche")
	FG_SK_2				= models.IntegerField(choices=[(i, i) for i in range(1, 6)], blank=True, null=True					, verbose_name="FG: Mein Gesprächspartner hat genau so gesprochen, wie er/sie auch sonst mit mir spricht")
	FG_SK_3				= models.IntegerField(choices=[(i, i) for i in range(1, 6)], blank=True, null=True					, verbose_name="FG: Ich hatte das Gefühl, dass mein Gesprächspartner und ich anders sprechen, als wir es sonst tun.")
	FG_SK_4				= models.IntegerField(choices=[(i, i) for i in range(1, 6)], blank=True, null=True					, verbose_name="FG: Dass das Gespräch aufgezeichnet wurde, habe ich während des Interviews schnell vergessen")
	FG_SK_5				= models.IntegerField(choices=[(i, i) for i in range(1, 6)], blank=True, null=True					, verbose_name="FG: Die Gesprächssituation war durch die besonderen Umstände (Mikrophon und anderes) unangenehm")
	FG_SK_6				= models.IntegerField(choices=[(i, i) for i in range(1, 6)], blank=True, null=True					, verbose_name="FG: Die Spielkarten haben das Gespräch interessant und anregend gemacht.")
	FG_SK_7				= models.IntegerField(choices=[(i, i) for i in range(1, 6)], blank=True, null=True					, verbose_name="FG: Ich hätte lieber ohne Spielkarten mit meinem Gegenüber geredet.")
	FG_SK_8				= models.IntegerField(choices=[(i, i) for i in range(1, 6)], blank=True, null=True					, verbose_name="FG: Die technischen Geräte (Mikrophone und anderes) haben mich gestört")
	FG_SK_9				= models.IntegerField(choices=[(i, i) for i in range(1, 6)], blank=True, null=True					, verbose_name="FG: Ich hatte den Eindruck, dass die technischen Geräte (Mikrophon und anderes) meinen Gesprächspartner in seinem Sprechen beeinflusst haben.")
	FG_SK_10			= models.IntegerField(choices=[(i, i) for i in range(1, 6)], blank=True, null=True					, verbose_name="FG: Ich habe mich während des Gesprächs wohl gefühlt.")
	FG_List				= models.CharField(max_length=511			, blank=True, null=True									, verbose_name="FG: Am besten haben mir folgende Spielkarten gefallen. (\"REDE ÜBER\", \"ERZÄHLE VON\", \"STELL DIR VOR\")")
	FG_Q_1				= models.TextField(							  blank=True, null=True									, verbose_name="FG: Meine Sprechweise/Sprechlage, die ich im Gespräch verwendet habe, nenne ich:")
	FG_Q_2				= models.TextField(							  blank=True, null=True									, verbose_name="FG: Haben Sie noch weitere Anregungen oder Verbesserungsvorschläge für uns?:")
	Comm_FB				= models.TextField(							  blank=True, null=True									, verbose_name="FG: Kommentar zum Erhebungsblatt")
