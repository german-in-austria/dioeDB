...
def transcript(request, aPk, aNr):
	...
	if tpk > 0:
		...
		if aNr == 0:
			...
			aInformanten = {}
			if aEinzelErhebungData:
				for aInf in aEinzelErhebungData.tbl_inf_zu_erhebung_set.all().values('ID_Inf').annotate(total=Count('ID_Inf')).order_by('ID_Inf'):
					aInfM = pdbmodels.tbl_informanten.objects.get(id=aInf['ID_Inf'])
					aInformanten[aInfM.pk] = {'k': aInfM.inf_sigle, 'ka': aInfM.inf_sigle, 'count': 0}
			for aInf in adbmodels.token.objects.filter(transcript_id_id=tpk).values('ID_Inf').annotate(total=Count('ID_Inf')).order_by('ID_Inf'):
				aInfM = pdbmodels.tbl_informanten.objects.get(id=aInf['ID_Inf'])
				aInformanten[aInfM.pk] = {'k': aInfM.inf_sigle, 'ka': aInfM.inf_sigle, 'count': aInf['total']}
