...
def view_start(request):
	...
	# Ausgabe der Seite
	return render_to_response('DB/start.html', RequestContext(request, {
		'tabellen': (tabellen.items()), 'database': settings.DATABASES['default']['ENGINE'],
		'info': info
	}),)
...
def view_view(request, app_name, tabelle_name):
	...
	if 'getlmfal' in request.POST:
		...
		return render_to_response('DB/lmfal.html', RequestContext(request, {
			'kategorien_liste': kategorienListe(amodel).items(), 'appname': app_name, 'tabname': tabelle_name,
			'info': info, 'error': error
		}),)

	# Liste der Einträge des jeweiligen Buchstaben ausgeben
	if 'getlmfadl' in request.POST:
		return render_to_response('DB/lmfadl.html', RequestContext(request, {
			'lmfadl': kategorienListe(amodel, inhalt=request.POST.get('getlmfadl')),
			'info': info, 'error': error
		}),)
	...
	if 'gettableview' in request.POST:
		...
		return render_to_response('DB/view_table.html', RequestContext(request, {
			'aelement': aElement, 'aelementapp': aElement._meta.app_label, 'aelementtabelle': aElement.__class__.__name__,
			'fields': felderAuslesen(aElement, 1), 'usedby': verbundeneElemente(aElement), 'amodel_meta': amodel._meta,
			'info': info, 'error': error
		}),)
	...
	if 'loadpk' in request.POST:
		...
		acontent = render_to_response('DB/view_table.html', RequestContext(request, {
			'aelement': aElement, 'aelementapp': aElement._meta.app_label, 'aelementtabelle': aElement.__class__.__name__,
			'fields': felderAuslesen(aElement, 1), 'usedby': verbundeneElemente(aElement), 'amodel_meta': amodel._meta,
			'info': info, 'error': error
		}),).content
		return render_to_response('DB/view.html', RequestContext(request, {
			'kategorien_liste': kategorienListe(amodel, mitInhalt=int(request.POST.get('loadpk')), arequest=request).items(),
			'appname': app_name, 'tabname': tabelle_name, 'amodel_meta': amodel._meta, 'acontent': acontent, 'amodel_count': amodel.objects.count(),
			'info': info, 'error': error
		}),)
	...
	if 'gettableeditform' in request.POST:
		...
		return render_to_response('DB/edit_table.html', RequestContext(request, {
			'aelement': aElement, 'aelementapp': aElement._meta.app_label, 'aelementtabelle': aElement.__class__.__name__, 'amodel_meta': amodel._meta,
			'aform': aform, 'pktitel': aElement._meta.pk.verbose_name, 'pkvalue': aElement.pk,
			'info': info, 'error': error
		}),)
	...
	if 'getforeignkeysel' in request.POST:
		try:
			...
			return render_to_response('DB/foreignkeysel.html', RequestContext(request, {
				'kategorien_liste': kategorienListe(amodel, mitInhalt=int(request.POST.get('getforeignkeysel')), arequest=request).items(),
				'appname': app_name, 'tabname': tabelle_name, 'amodel_meta': amodel._meta, 'acontent': acontent,
				'info': info, 'error': error
			}),)
		except ObjectDoesNotExist:
			return render_to_response('DB/foreignkeysel.html', RequestContext(request, {
				'kategorien_liste': kategorienListe(amodel).items(), 'appname': app_name, 'tabname': tabelle_name, 'amodel_meta': amodel._meta, 'acontent': '',
				'info': info, 'error': error
			}),)
	...
	if 'delobj' in request.POST and request.user.has_perm(app_name + '.edit'):
		...
		return render_to_response('DB/view_empty.html', RequestContext(request, {
			'amodel_meta': amodel._meta, 'appname': app_name, 'tabname': tabelle_name,
			'info': info, 'error': error
		}),)
	...
	return render_to_response('DB/view.html', RequestContext(request, {
		'kategorien_liste': kategorienListe(amodel).items(), 'appname': app_name, 'tabname': tabelle_name,
		'amodel_meta': amodel._meta, 'amodel_count': amodel.objects.count(),
		'info': info, 'error': error
	}),)
