/* global $ jQuery setSearchfields makeScrollTo getLmfadl loadElement asurl makeModal csrf viewurl setMaps alert post onSelobjModal aappname atabname onSeleobjbtnnone onSeleobjbtn onSeleobjSelbtn aktueb onSelobjOsmModal onSeleosmbtnnone onSeleosmbtn secondsToDuration durationToSeconds fxinputs resetEinblendbareFelder confirm reloadMenu */
...
(function ($) {
	jQuery(document).ready(function ($) {
		...
		$(document).on('click', '.element-reihung-hoch , .element-reihung-runter', function (e) {	/* Reihung rauf/runter */
			var aformdata = $(this).closest('.formdata');
		...
		$(document).on('click', '.element-hinzufuegen', function (e) {	/* Element hinzufügen */
			necount = necount + 1;
			var newbase = $(this).siblings('.newbase');
			var newclone = newbase.clone().removeClass('hidden newbase');
		...
		function formSaveData (athis) {	/* Formular Daten, zum speichern, auswerten */
			...
			$(athis).progenyWithoutAncestors('.form-fields', '.form-list').children('.formdata:not(.newbase)').each(function () {
				var acontb = $(this).children().progenyWithoutAncestors('.formdata', '.form-fields');
		...
		$(document).on('click', '.form-save', function (e) {	/* Formular speichern */
			...
			$('.formdata:not(.hidden) [data-errorcheck]').each(function () {
				...
				$.each($(this).data('errorcheck'), function (a, b, c, d, aobj = aelement) {
					var gVal = aelement.val();
					var sVal = aelement.closest('.form-group').find('[name="' + this['field'] + '"]').val();
